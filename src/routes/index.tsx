...
function HomePage() {
  ...
  return <><SiteHeader /><main>
    ...
    ].map(([Icon, title, text]) => <div key={String(title)} className="rounded-2xl border border-border bg-card p-7 shadow-sm"><span className="grid size-12 place-items-center rounded-xl bg-secondary/25 text-primary"><Icon /></span><h3 className="mt-5 text-lg font-bold">{String(title)}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{String(text)}</p></div>)}</div></div></section>
