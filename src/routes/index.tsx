...
function HomePage() {
  ...
  return <><SiteHeader /><main>
    ...
    <section className="bg-background py-20"><div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="grid gap-6 md:grid-cols-3">{[
      [ShieldCheck, "Drive with confidence", "Every car is inspected and includes clear protection options."], [CalendarCheck, "Plans stay flexible", "Change or cancel eligible bookings without hidden surprises."], [Headphones, "Help when you need it", "Friendly support is ready throughout your rental journey."],
    ].map(([Icon, title, text]) => <div key={title} className="rounded-2xl border border-border bg-card p-7 shadow-sm"><span className="grid size-12 place-items-center rounded-xl bg-secondary/25 text-primary">{Icon === ShieldCheck ? <ShieldCheck /> : Icon === CalendarCheck ? <CalendarCheck /> : <Headphones />}</span><h3 className="mt-5 text-lg font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{text}</p></div>)}</div></div></section>
